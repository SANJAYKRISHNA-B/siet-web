...
const icon = (name) => `<span class="ui-icon" aria-hidden="true">${({ home: '⌂', users: '♙', book: '▤', grad: '◇', building: '▥', quality: '✓', brief: '▣', compass: '◉', arrow: '→', down: '⌄', menu: '☰', close: '×', trophy: '♛', trend: '↗', gift: '◇', play: '▶', phone: '☎' }[name] || '◆')}</span>`;
...
<header class="institution-header-v4 exact-image-header"><div class="institution-header-shell"><a class="siet-header-image" href="#/" aria-label="Sri Shakthi Institute of Engineering and Technology home"><img src="/brand/siet-exact-header.png" alt="Sri Shakthi Institute of Engineering and Technology — NBA accredited, NAAC A grade, counselling code 2727" width="2048" height="256"></a><nav class="institution-navbar" aria-label="Main navigation"><a class="institution-home" href="#/" aria-label="Home">${icon('home')}</a><button class="institution-mobile-toggle" aria-label="Open menu">${icon('menu')}</button><div class="institution-menu">${pageGroups.map((g, i) => `${i === 5 ? '<a class="institution-nav-link" href="#/placements">Placements</a>' : ''}<div class="institution-nav-group"><button type="button">${g.label}${icon('down')}</button><div>${g.items.map(([s, n]) => `<a href="#/${s}">${n}</a>`).join('')}</div></div>`).join('')}<a class="institution-nav-link" href="#/careers">Careers</a></div></nav></div></header>
<div class="mobile-nav"><a class="mobile-apply-link" href="#/apply">Apply Now</a><a href="#/placements">Placements</a><a href="#/careers">Careers @ SIET</a>${pageGroups.flatMap(g => g.items.slice(0, 2)).map(([s, n]) => `<a href="#/${s}">${n}</a>`).join('')}</div>`
}
...
<section class="campus-section"><div class="campus-container"><aside class="campus-left reveal"><div class="section-kicker"><span>03</span><i></i><span>LIFE AT SRI SHAKTHI</span></div><h1>Campus Moments.<em>Student stories.</em></h1><p>Explore learning, innovation, celebrations and everyday campus experiences from the Sri Shakthi community.</p><a class="button" href="#/campus-life">Explore campus ${icon('arrow')}</a></aside><main class="campus-content"><div class="campus-gallery">${[['placements.png', 'A campus that inspires every day.'], ['student-life.png', 'Victory is a habit here.'], ['cultural.png', 'Culture. Tradition. Every performance.'], ['innovation.png', 'Ideas that create impact.']].map(([img, t]) => `<a class="gallery-card reveal" href="#/campus-life"><img src="/brand/campus-life/${img}" alt="${t}"><div class="gallery-content"><h3>${t}</h3><span>→</span></div></a>`).join('')}</div></main></div></section></main>`
}

function programmeCards(list) { return list.map(([n, d]) => `<a class="programme-card programme-card-simple reveal" href="#/department/${slugify(n)}"><span class="programme-icon">◆</span><h4>${n}</h4><span class="arrow-btn">→</span></a>`).join('') }
...
function internalPage(route) { let dept = ''; let desc = 'Build strong engineering foundations through expert teaching, practical laboratories, industry exposure, projects, research and collaborative learning.'; if (route.startsWith('department/')) { const slug = route.slice(11); const match = [...ugPrograms, ...pgPrograms].find(([n]) => slugify(n) === slug); if (match) { dept = match[0]; desc = match[1]; } else { dept = titleCase(slug.replaceAll('-', ' ')).replaceAll(' And ', ' & '); } } const data = dept ? [dept, `Department of ${dept}`, desc] : (pageCopy[route] || ['Sri Shakthi', 'Institutional information', 'Explore Sri Shakthi Institute of Engineering and Technology.']); return `<main class="internal-page"><section class="page-hero"><img class="page-crest" src="/brand/siet-logo.png" alt=""><div class="eyebrow"><span></span> SRI SHAKTHI</div><h1 class="reveal">${data[0]}</h1><p>${data[1]}</p></section><section class="page-content"><div class="reveal"><div class="section-no">OVERVIEW</div><h2>${data[1]}</h2><p>${data[2]}</p>${route === 'contact' ? '<div class="contact-panel"><b>Sri Shakthi Institute of Engineering & Technology</b><p>Sri Shakthi Nagar, L&T By-Pass, Chinniyampalayam, Coimbatore – 641062</p><p>+91 422 2369900 · info@siet.ac.in</p></div>' : ''}<a class="button" href="#/admission-enquiry">Enquire now ${icon('arrow')}</a></div></section>${['departments', 'programmes'].includes(route) ? `<section class="page-content programme-content"><div class="section-no">PROGRAMMES &amp; DEPARTMENTS</div><div>${[...ugPrograms, ...pgPrograms].map(([n, d, img]) => `<a class="flip-card" href="#/department/${slugify(n)}"><span class="flip-card-inner"><span class="flip-front"><small>DEPARTMENT</small><b>${n}</b><p>${d}</p><span>Explore department →</span></span><span class="flip-back" style="background-image:linear-gradient(180deg,transparent,rgba(3,45,27,.94)),url('${img}')"><b>${n}</b></span></span></a>`).join('')}</div></section>` : ''}</main>` }
const titleCase = s => s.replace(/\b\w/g, c => c.toUpperCase());

function enquiryPage(apply = false) { return `<main class="enquiry-page-v3"><section class="enquiry-hero-v3"><img src="/brand/siet-logo.png" alt=""><div class="eyebrow"><span></span> SRI SHAKTHI</div><h1>${apply ? 'Apply for Sri Shakthi' : 'Admission Enquiry'}</h1><h2>Let’s plan your <em>next step.</em></h2><p>Share your interests and contact details. Our admissions team will guide you in choosing the right programme.</p></section><section class="enquiry-main-v3"><div class="enquiry-heading-v3"><small>ENQUIRY FORM</small><h1>Start your engineering journey with SIET</h1></div><form class="enquiry-form-v3 js-form"><div class="enquiry-fields-v3">${field('Full Name', 'name', 'text', 'Enter your full name')}${field('Mobile Number', 'phone', 'tel', 'Enter 10 digit mobile number')}${field('Email Address', 'email', 'email', 'Enter your email address')}${selectField('Course Level', 'level', ['UG', 'PG'])}${selectField('Preferred Department', 'course', [...ugPrograms, ...pgPrograms].map(p => p[0]))}${field('Academic Qualification / Marks', 'qualification', 'text', 'Qualification and marks')}</div><label>Message / Any Specific Query <b>*</b><textarea name="message" rows="4" required minlength="10"></textarea></label><button class="button" type="submit">${apply ? 'Submit Application' : 'Send Enquiry'} →</button><p class="status" aria-live="polite"></p></form></section></main>` }
...
function render() { const r = route(); let content = !r ? homePage() : r === 'vision-mission' || r === 'about' ? visionPage() : r === 'chairman' ? chairmanPage() : r === 'principal' ? principalPage() : r === 'admission-enquiry' || r === 'apply' ? enquiryPage(r === 'apply') : r === 'careers' ? careersPage() : internalPage(r); $('#app').innerHTML = header() + content + footer(); document.title = `${r ? titleCase(r.replaceAll('-', ' ')) : 'Sri Shakthi'} | SIET`; bind(); scrollTo(0, 0) }

function bind() {
    const mobile = $('.mobile-nav'), toggle = $('.institution-mobile-toggle'); toggle?.addEventListener('click', () => { mobile.classList.toggle('open'); toggle.innerHTML = mobile.classList.contains('open') ? icon('close') : icon('menu') }); $$('.mobile-nav a').forEach(a => a.addEventListener('click', () => mobile.classList.remove('open')));
    ...
    $$('.toggle-btn').forEach(b => b.addEventListener('click', () => { $$('.toggle-btn').forEach(x => x.classList.toggle('active', x === b)); $('#level-name').textContent = b.dataset.level; $('#programme-grid').innerHTML = programmeCards(b.dataset.level === 'UG' ? ugPrograms : pgPrograms); observe() }));
...
addEventListener('hashchange', render); addEventListener('keydown', e => { if (e.key === 'Escape') $('.video-close')?.click() }); render();
